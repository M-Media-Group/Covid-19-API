...
countries_url = 'https://raw.githubusercontent.com/M-Media-Group/country-json/master/src/country-by-population.json'
...
for row in recent_rows:
    ...
    recent_data[row['Country_Region']][row['Province_State']]['deaths'] = int(
        row['Deaths'])
    recent_data[row['Country_Region']][
        row['Province_State']]['Last_Update'] = datetime.datetime.strptime(
...
def lambda_handler(event, context):
    ...
    url = 'https://raw.githubusercontent.com/CSSEGISandData/COVID-19/master/csse_covid_19_data/csse_covid_19_time_series/time_series_19-covid-' + event[
        'queryStringParameters']['status'].capitalize() + '.csv'
    ...
    for row in rows:
        ...
        if (row['Country/Region'] in recent_data and row['Province/State']
                in recent_data[row['Country/Region']]):
            data[row['Country/Region']][
                row['Province/State']]['lat'] = row['Lat']
            data[row['Country/Region']][
                row['Province/State']]['long'] = row['Long']
            ...
            data[row['Country/Region']]['All']['confirmed'] = int(
                recent_data[row['Country/Region']][
                    row['Province/State']]['confirmed']) + int(
                        data[row['Country/Region']]['All']['confirmed'])
            data[row['Country/Region']]['All']['recovered'] = int(
                recent_data[row['Country/Region']][
                    row['Province/State']]['recovered']) + int(
                        data[row['Country/Region']]['All']['recovered'])
            data[row['Country/Region']]['All']['deaths'] = int(recent_data[
                row['Country/Region']][row['Province/State']]['deaths']) + int(
                    data[row['Country/Region']]['All']['deaths'])

        if ('confirmed'
                not in data[row['Country/Region']][row['Province/State']]):
            if (row['Country/Region'] in recent_data and row['Province/State']
                    in recent_data[row['Country/Region']]):
                data[row['Country/Region']][row['Province/State']][
                    'confirmed'] = recent_data[row['Country/Region']][
                        row['Province/State']]['confirmed']
                data[row['Country/Region']][row['Province/State']][
                    'recovered'] = recent_data[row['Country/Region']][
                        row['Province/State']]['recovered']
                data[row['Country/Region']][
                    row['Province/State']]['deaths'] = recent_data[
                        row['Country/Region']][row['Province/State']]['deaths']
                data[row['Country/Region']][row['Province/State']][
                    'last_update'] = recent_data[row['Country/Region']][
                        row['Province/State']]['Last_Update']

            else:
                data[row['Country/Region']][
                    row['Province/State']]['confirmed'] = 00
                data[row['Country/Region']][
                    row['Province/State']]['recovered'] = 00
                data[row['Country/Region']][
                    row['Province/State']]['deaths'] = 00
                data[row['Country/Region']][
                    row['Province/State']]['last_update'] = "N/A"
