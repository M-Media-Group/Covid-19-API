    ...
    if not os.path.isfile('/tmp/history-'+status+'.json'):
        ...
        for row in rows:
            ...
            if (row['Country/Region'] not in data):
                ...
                if(row['Country/Region'] in country_array):
                    for key in country_array[row['Country/Region']]:
                        data[row['Country/Region']
                             ]['All'][key] = country_array[row['Country/Region']][key]
            ...
            for column in reversed(row):
                if column != 'Province/State' and column != 'Country/Region' and column != 'Lat' and column != 'Long':
                    ...
                    data[row['Country/Region']]['All']['dates'][column] = data[row['Country/Region']
                                                                               ]['All']['dates'][column] + int(float(row[original_column]))
                    ...
                    data[row['Country/Region']][row['Province/State']
                                                ]['dates'][column] = int(float(row[original_column]))

        global_array = {
            'All': {'population': country_array['Global']['population'], 'dates': {}}}
        ...
        for (key, value) in data.items():
            ...
            for column in value['All']['dates']:
                ...
                global_array['All']['dates'][column] = global_array['All']['dates'][column] + \
                    value['All']['dates'][column]
...
def proccessLatest():
    ...
    for row in recent_rows:
        ...
        country_array['Global']['confirmed'] = country_array['Global']['confirmed'] + \
            int(row['Confirmed'])
        country_array['Global']['deaths'] = country_array['Global']['deaths'] + \
            int(row['Deaths'])
        country_array['Global']['recovered'] = country_array['Global']['recovered'] + \
            int(row['Recovered'])

        if(row['Country_Region'] in country_array):
            for key in country_array[row['Country_Region']]:
                recent_data[row['Country_Region']
                            ]['All'][key] = country_array[row['Country_Region']][key]
        ...
        if(row['Province_State'] != 'All'):
            recent_data[row['Country_Region']]['All']['confirmed'] = int(
                recent_data[row['Country_Region']][row['Province_State']]['confirmed']) + int(recent_data[row['Country_Region']]['All']['confirmed'])
            recent_data[row['Country_Region']]['All']['recovered'] = int(
                recent_data[row['Country_Region']][row['Province_State']]['recovered']) + int(recent_data[row['Country_Region']]['All']['recovered'])
            recent_data[row['Country_Region']]['All']['deaths'] = int(
                recent_data[row['Country_Region']][row['Province_State']]['deaths']) + int(recent_data[row['Country_Region']]['All']['deaths'])

    global_array = {'All': {'population': country_array['Global']['population'], 'confirmed': country_array['Global']
                            ['confirmed'], 'recovered': country_array['Global']['recovered'], 'deaths': country_array['Global']['deaths']}}
...
def getDateWithFormat():
    # get the yesterdays date and format it in mm-dd-yyyy
    yesterday = datetime.date.today() - datetime.timedelta(days=1)
    return yesterday.strftime('%m-%d-%Y')


def proccessLatestFromJHU():
    url = 'https://raw.githubusercontent.com/CSSEGISandData/COVID-19/master/csse_covid_19_data/csse_covid_19_daily_reports/' + \
        getDateWithFormat()+'.csv'
    recent_data = {}
    ...
    recent_rows = getRowsFromCsv(url)
    ...
    if not os.path.isfile('/tmp/vaccines.json'):
        ...
        for row in recent_rows:
            ...
            if (row['Country_Region'] not in vaccine_data):
                ...
                vaccine_data[row['Country_Region']
                             ]['All']['people_vaccinated'] = 0
                vaccine_data[row['Country_Region']
                             ]['All']['people_partially_vaccinated'] = 0

            country_array['Global']['administered'] = (
                country_array['Global']['administered'] + int(float(row['Doses_admin']))) if row['Doses_admin'] else 0
            country_array['Global']['people_partially_vaccinated'] = country_array['Global']['people_partially_vaccinated'] + \
                int(float(row['People_partially_vaccinated'])
                    ) if row['People_partially_vaccinated'].strip() != "" else 0
            country_array['Global']['people_vaccinated'] = country_array['Global']['people_vaccinated'] + int(
                float(row['People_fully_vaccinated'])) if row['People_fully_vaccinated'].strip() != "" else 0
            ...
            vaccine_data[row['Country_Region']][row['Province_State']]['administered'] = int(
                float(row['Doses_admin'])) if row['Doses_admin'] else 0
            vaccine_data[row['Country_Region']][row['Province_State']]['people_vaccinated'] = int(
                float(row['People_fully_vaccinated'])) if row['People_fully_vaccinated'].strip() != "" else 0
            vaccine_data[row['Country_Region']][row['Province_State']]['people_partially_vaccinated'] = int(
                float(row['People_partially_vaccinated'])) if row['People_partially_vaccinated'].strip() != "" else 0
            # vaccine_data[row['Country_Region']][row['Province_State']]['updated'] = row['Date']
            vaccine_data[row['Country_Region']][row['Province_State']]['updated'] = datetime.datetime.strptime(
                row['Date'], '%Y-%m-%d').strftime('%Y/%m/%d %H:%M:%S+00')

        global_array = {'All': {'population': country_array['Global']['population'], 'administered': country_array['Global']['administered'],
                                'people_vaccinated': country_array['Global']['people_vaccinated'], 'people_partially_vaccinated': country_array['Global']['people_partially_vaccinated']}}
...
def lambda_handler(event, context):
    ...
    try:
        if ('ab' in event['queryStringParameters']):
            return_data = list(filter(lambda x: 'abbreviation' in x['All'] and event['queryStringParameters']['ab'].upper(
            ) == x['All']['abbreviation'], data.values()))[0]
        elif ('continent' in event['queryStringParameters']):
            return_data = {k: v for k, v in data.items() if 'continent' in v['All'] and v['All']['continent'].upper(
            ) == event['queryStringParameters']['continent'].upper()}
        else:
