...
def proccessMain(status):
    ...
    if not os.path.isfile('/tmp/history-' + status + '.json'):
        ...
        for row in rows:
            ...
            if (row['Country/Region'] not in data):
                ...
                if (row['Country/Region'] in country_array):
                    for key in country_array[row['Country/Region']]:
                        data[row['Country/Region']]['All'][
                            key] = country_array[row['Country/Region']][key]
            ...
            for column in reversed(row):
                if column != 'Province/State' and column != 'Country/Region' and column != 'Lat' and column != 'Long':
                    ...
                    if (column not in data[row['Country/Region']]['All']
                        ['dates']):
        ...
        global_array = {
            'All': {
                'population': country_array['Global']['population'],
                'dates': {}
            }
        }
...
def proccessLatest():
    ...
    for row in recent_rows:
        ...
        country_array['Global']['confirmed'] = country_array['Global'][
            'confirmed'] + int(row['Confirmed'])
        country_array['Global'][
            'deaths'] = country_array['Global']['deaths'] + int(row['Deaths'])
        country_array['Global']['recovered'] = country_array['Global'][
            'recovered'] + int(row['Recovered'])

        if (row['Country_Region'] in country_array):
            for key in country_array[row['Country_Region']]:
                recent_data[row['Country_Region']]['All'][key] = country_array[
                    row['Country_Region']][key]
        ...
        recent_data[row['Country_Region']][
            row['Province_State']]['deaths'] = int(row['Deaths'])
        recent_data[row['Country_Region']][
            row['Province_State']]['updated'] = row['Last_Update']
        ...
        if (row['Province_State'] != 'All'):
            ...
            recent_data[row['Country_Region']]['All']['deaths'] = int(
                recent_data[row['Country_Region']][
                    row['Province_State']]['deaths']) + int(
                        recent_data[row['Country_Region']]['All']['deaths'])

    global_array = {
        'All': {
            'population': country_array['Global']['population'],
            'confirmed': country_array['Global']['confirmed'],
            'recovered': country_array['Global']['recovered'],
            'deaths': country_array['Global']['deaths']
        }
    }
...
def lambda_handler(event, context):
    ...
    try:
        if ('ab' in event['queryStringParameters']):
            return_data = list(
                filter(
                    lambda x: 'abbreviation' in x[
                        'All'] and event['queryStringParameters']['ab'].upper(
                        ) == x['All']['abbreviation'], data.values()))[0]
        elif ('continent' in event['queryStringParameters']):
            return_data = {
                k: v
                for k, v in data.items()
                if 'continent' in v['All'] and v['All']['continent'].upper() ==
